...
| R3 | Coste manual en MVP | En MVP el coste operativo se registra de forma manual en cada actividad/registro aplicable. No hay cálculo automático obligatorio en esta fase. |
...
| R5 | Cosecha: kg obligatorio + campos netos opcionales excluyentes | El registro de cosecha siempre incluye peso_bruto_kg (obligatorio). Los campos `rendimiento` y `litros_obtenidos` son opcionales, pero no pueden informarse ambos en el mismo registro. |
...
| 2 | Registro de Actividades y Recursos con maestro de Trabajadores | Qué se hizo, quién, cuándo, en qué terreno, duración. Responsable = entidad Trabajador del Workspace (puede tener cuenta o no). Coste manual en registro operativo para MVP. |
| 3 | Gestión de Producción | Producto, peso bruto kg obligatorio y campos netos (`rendimiento`, `litros_obtenidos`) opcionales y excluyentes entre sí. Terreno fuente vinculado al Workspace. |
...
- El widget de destinos incluye la categoría canónica `desconocido` (alias visual permitido: "Sin destino").
