...
| Evolución rendimiento (MVP-404) | `GET /api/v1/dashboard/yield-evolution` | `season_id?`, `plot_ids?[]`, `granularity?=month\|week` | `200 { scope, granularity, data:[{ period, yield_l_per_100kg, kg }], history }` |
...
| `yield-evolution` — `history` (MVP-404) | Comparativa histórica básica (RN-015): `{ average, average_5_seasons, average_10_seasons, prior_seasons_with_data }`. Compara los **mismos terrenos** en temporadas anteriores (el filtro de terreno viaja al histórico). Cada media es `null` mientras no haya «histórico suficiente»: la general con una temporada previa con dato; las de 5 y 10, con al menos 5 y 10. Se cuenta sobre temporadas **con dato**, no sobre el calendario; «previa» es por fecha de inicio. En el MVP una temporada es la campaña anual, así que 5 temporadas ≈ 5 años |
