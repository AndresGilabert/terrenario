...
| P-005 | 2026-07-25 | MVP-001 / MVP-105 | seguridad | Headers de seguridad HTTP obligatorios por `docs/07-seguridad/autenticacion-autorizacion.md` (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Content-Security-Policy`) no implementados en `Program.cs` y no asignados a ninguna historia. Detectado al cerrar el perimetro de seguridad en MVP-105. Es transversal a todos los endpoints y requiere validar que la CSP no rompa frontend/OpenAPI, por lo que no se incluye en MVP-105 para no ampliar su alcance. Definir story: middleware de headers + CSP afinada + verificacion. | medio | no | MVP-999 | pendiente | - |
| P-006 | 2026-07-25 | MVP-001 / MVP-105 | observabilidad | `X-Request-Id` en todas las respuestas, exigido por las convenciones de `docs/02-arquitectura/contratos-api.md` (trazabilidad y correlacion de errores 500), no implementado ni asignado. Transversal a toda la API; encaja de forma natural con la observabilidad de MVP-006 pero no bloquea. Definir story: middleware que genere/propague el id y lo incluya en logs y respuesta. | medio | no | MVP-999 | pendiente | - |
| P-007 | 2026-07-25 | MVP-001 / MVP-105 | tecnico | Cliente HTTP comun en el frontend con manejo centralizado de 401/403 de scope. Hoy cada `*.service.ts` hace `fetch` a mano. Con el enforcement introducido en MVP-105, cuando lleguen recursos con ambito de Workspace (MVP-002+) conviene un cliente unico que reaccione a `AUTH_WORKSPACE_SCOPE_REQUIRED` (forzar onboarding/seleccion de Workspace) y `AUTH_WORKSPACE_FORBIDDEN` (refrescar contexto), ademas del refresh de token ya existente. Diferible hasta que exista la primera pantalla de recurso con scope. | bajo | no | MVP-999 | pendiente | - |
