...
/// como 404 las invitaciones no reenviables (otro Workspace, canal enlace o no pendiente).
/// </summary>
public class ResendInvitationHandlerTests
{
    ...
    [Fact]
    public async Task Deberia_Ocultar_Cuando_ElCanalEsEnlace()
    {
        // Arrange — el canal enlace no tiene destinatario: no se reenvía a una persona
        ...
        // Act
        var act = async () => await sut.HandleAsync(CommandFor(link.Id, deliverEmail: true));
